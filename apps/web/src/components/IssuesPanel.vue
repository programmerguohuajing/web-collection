    ...
    <el-table :data="issues" size="small" empty-text="暂无错误">
      <el-table-column label="错误信息" min-width="260">
        <template #default="{ row }">
          <el-tooltip :content="row.message || '-'" placement="top" :append-to="() => document.body"><span class="table-ellipsis">{{ row.message || '-' }}</span></el-tooltip>
        </template>
      </el-table-column>
      ...
      <el-table-column label="源码位置" min-width="220">
        <template #default="{ row }">
          <el-tooltip :content="sourceLabel(row.original)" placement="top" :append-to="() => document.body"><span class="table-ellipsis">{{ sourceLabel(row.original) }}</span></el-tooltip>
        </template>
      </el-table-column>
