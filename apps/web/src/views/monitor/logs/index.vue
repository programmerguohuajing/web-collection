...
<template>
  ...
  <el-card shadow="never" class="section panel">
    ...
    <el-table :data="rows" border>
      ...
      <el-table-column label="Trace" min-width="180" show-overflow-tooltip><template #default="{ row }">{{ row.traceId ? (`/traces?traceId=${encodeURIComponent(row.traceId)}`) : '' }}</template></el-table-column>
    </el-table>
