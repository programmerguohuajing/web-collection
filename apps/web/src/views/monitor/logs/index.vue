...
<template>
  ...
  <el-card shadow="never" class="section panel">
    ...
    <el-table v-loading="loading" :data="rows" border>
      ...
      <el-table-column label="内容" min-width="320"><template #default="{ row }"><el-tooltip :content="row.message" placement="top" :append-to="() => document.body"><span class="table-ellipsis">{{ row.message }}</span></el-tooltip></template></el-table-column>
      ...
      <el-table-column label="会话" min-width="180"><template #default="{ row }"><el-tooltip :content="row.sessionId || ''" placement="top" :append-to="() => document.body"><span class="table-ellipsis">{{ row.sessionId || '' }}</span></el-tooltip></template></el-table-column>
      <el-table-column label="Trace" min-width="180"><template #default="{ row }"><el-tooltip :content="row.traceId || ''" placement="top" :append-to="() => document.body"><span class="table-ellipsis">{{ row.traceId ? (`/traces?traceId=${encodeURIComponent(row.traceId)}`) : '' }}</span></el-tooltip></template></el-table-column>
    </el-table>
