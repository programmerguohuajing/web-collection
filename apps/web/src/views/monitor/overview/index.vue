...
<template>
  ...
  <el-card shadow="never" class="panel section activity-panel">
    ...
    <el-table :data="activityRows" empty-text="暂无活动数据">
      ...
      <el-table-column label="页面 / 接口" min-width="210"><template #default="{ row }"><el-tooltip :content="row.path || row.url || '-'" placement="top"><span class="table-ellipsis">{{ row.path || row.url || '-' }}</span></el-tooltip></template></el-table-column>
