...
const pathExpandedKeys = ref(new Set())
let listRequestId = 0
...
function pathExpanded(row) {
  return pathExpandedKeys.value.has(row.session_id)
}

function togglePath(row) {
  const set = new Set(pathExpandedKeys.value)
  if (set.has(row.session_id)) set.delete(row.session_id)
  else set.add(row.session_id)
  pathExpandedKeys.value = set
}
      ...
      <el-table-column label="访问页面" min-width="240" cell-class-name="no-ellipsis">
        <template #default="{ row }">
          <span v-if="row.paths.length" class="path-cell">
            <template v-for="(p, i) in (pathExpanded(row) ? row.paths : visiblePaths(row))" :key="i">
              <span class="path-crumb" :title="p">{{ p }}</span>
              <el-icon v-if="i < ((pathExpanded(row) ? row.paths.length : visiblePaths(row).length) - 1)" class="path-crumb-arrow"><ArrowRight /></el-icon>
            </template>
            <span v-if="row.paths.length > PATH_PREVIEW" class="path-toggle" :class="{ 'is-expanded': pathExpanded(row) }" @click.stop="togglePath(row)">
              {{ pathExpanded(row) ? '收起' : `+${row.paths.length - PATH_PREVIEW} 页` }}
              <el-icon class="path-toggle-arrow"><ArrowDown v-if="!pathExpanded(row)" /><ArrowUp v-else /></el-icon>
            </span>
          </span>
          <span v-else>-</span>
        </template>
