...
<template>
  ...
  <el-card shadow="never" class="section panel">
    ...
    <el-table v-loading="loading" :data="rows" border @row-click="viewSession" style="cursor:pointer">
      <el-table-column label="会话 ID" min-width="200"><template #default="{ row }"><el-tooltip :content="row.session_id" placement="top"><span class="table-ellipsis">{{ row.session_id }}</span></el-tooltip></template></el-table-column>
      <el-table-column prop="user_id" label="用户 ID" width="180"><template #default="{ row }"><el-tooltip :content="row.user_id" placement="top"><span class="table-ellipsis">{{ row.user_id }}</span></el-tooltip></template></el-table-column>
      ...
      <el-table-column prop="paths" label="访问页面" min-width="260"><template #default="{ row }"><el-tooltip :content="(row.paths || []).join(' → ')" placement="top"><span class="table-ellipsis">{{ (row.paths || []).join(' → ') }}</span></el-tooltip></template></el-table-column>
    </el-table>
