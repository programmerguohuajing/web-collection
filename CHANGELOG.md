...
- **AI 洞察未读通知治理**：引入 `markInsightsAsRead` 与 `sinceTs` 时间戳过滤，根治重复触发未读通知红点问题。
- **合规 DSR 全流程强化**：完成合规 DSR 工单全流程验证、草稿编辑更新、Admin 自批及全覆盖自动化测试套件。

### ✨ 新功能 (Features)
- **ui**：侧边栏全量替换为纯线条描边图标并保证 36 个菜单图标完全唯一（`0b5856a`、`b2515ae`）
- **insights**：AI 洞察未读通知引入已读时间戳持久化与增量未读查询（`62649bc`）

### 🐛 缺陷修复 (Fixes)
- **dsr**：合规 DSR 草稿编辑更新、自审批限制与路由导航闭环修复（`a92cc77`）
