 ...
 *
 * Electron 端红线：Web 端 SDK 抛异常最坏是采集中断，**Electron 主进程未捕获异常
 * 会触发 `uncaughtException` 甚至直接崩溃退出**，preload 抛错会导致渲染进程白屏。
...
/**
 * 创建模块熔断器（L3）：同一模块**连续** N 次异常即永久关闭，避免主进程持续刷异常与 CPU 占用。
 * @param {{ label?: string, diagnostics?: { emit?: Function } | null, maxFailures?: number }} [options={}]
 * @returns {{ disabled: boolean, failures: number, recordSuccess: () => void,
 *   recordFailure: (detail?: object) => void, reset: () => void,
