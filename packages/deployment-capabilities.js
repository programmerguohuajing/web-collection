 ...
 *
 * 例外（2026-09-14 产品决定）：insights（事件分析）在产品分析页改为 v-if 门禁——
 * 不支持时不渲染该 Tab，而不是留一个永远禁用、点不动的入口。
 * 理由：Worker 无 /api/analytics/insights 端点且长期不会镜像（Node 侧已完整实现），
 * 禁用态 Tab 对生产用户只是噪声。**能力位本身不变**（Worker 仍 false，Node 仍 true），
 * 仅前端不再渲染禁用态入口。回归守卫见 test/web-analytics-insights-tab.test.js。
 */
