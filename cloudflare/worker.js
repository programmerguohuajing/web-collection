...
async function record(env, event, application) {
  ...
  if (event.type === 'replay') {
    ...
    if (event.sessionId && events.length) await storageWrite(env, `insert into replays (session_id,app_id,user_id,user_name,user_phone,created_at,url,release_name,end_reason,events_json,base_session_id) values (?,?,?,?,?,?,?,?,?,?,?)`, [event.sessionId,event.appId,event.userId,event.userName,event.userPhone,event.ts,event.url,event.release,event.segmentEndReason||null,JSON.stringify(events),event.baseSessionId||null])
    return true
  }
...
async function replayEvents(env,id){if(!id?.trim())return json([]);let rows=(await env.DB.prepare('select events_json from replays where session_id=? order by created_at,id').bind(id).all()).results;if(!rows.length)rows=(await env.DB.prepare('select events_json from replays where session_id like ? order by created_at,id').bind(`${id}%`).all()).results;if(!rows.length)rows=(await env.DB.prepare('select events_json from replays where base_session_id=? order by created_at,id').bind(id).all()).results;if(!rows.length)rows=(await env.DB.prepare('select events_json from replays where base_session_id like ? order by created_at,id').bind(`${id}%`).all()).results;return json(rows.flatMap(row=>parse(row.events_json,[])))}
