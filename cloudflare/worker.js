...
async function record(env, event, application) {
  ...
  if (rules.blockedTypes?.includes(event.type) || rules.blockedNames?.includes(event.name) || (rules.allowedOrigins?.length && !rules.allowedOrigins.includes(origin(event.url)))) return false
...
async function adminApi(request, env, url) {
  ...
  if (/^\/api\/traces\//.test(path)) return traceEvents(env, decodeURIComponent(path.split('/').at(-1)))
  if (path === '/api/analytics/sessions') return sessions(env, url)
  if (/^\/api\/analytics\/sessions\//.test(path)) return sessionEvents(env, decodeURIComponent(path.split('/').at(-1)))
  ...
  if (path === '/api/funnels' && request.method === 'GET') return json((await env.DB.prepare('select * from funnels order by updated_at desc').all()).results.map(row => ({...row,steps_json:parse(row.steps_json,[])})))
...
async function paged(env, table, url, order) {
  const page=Math.max(1,Number(url.searchParams.get('page')||1)), pageSize=Math.min(100,Math.max(1,Number(url.searchParams.get('pageSize')||10)))
  const appId=url.searchParams.get('appId'), status=url.searchParams.get('status'), parts=[], values=[]
  if(appId){parts.push('app_id=?');values.push(appId)} if(status){parts.push('status=?');values.push(status)} const where=parts.length?`where ${parts.join(' and ')}`:''
  const [rows,total]=await Promise.all([env.DB.prepare(`select * from ${table} ${where} order by ${order} desc limit ? offset ?`).bind(...values,pageSize,(page-1)*pageSize).all(),env.DB.prepare(`select count(*) count from ${table} ${where}`).bind(...values).first()])
  return json({items:rows.results.map(mapIssue),total:total.count,page,pageSize})
}

async function summary(env,url){const {where,values}=filters(url);const rows=(await env.DB.prepare(`select * from events ${where} order by ts desc limit 5000`).bind(...values).all()).results.map(mapEvent);const issues=(await env.DB.prepare('select * from issues order by last_seen desc limit 100').all()).results;const byType={},behavior={},perf={};const perfRows=rows.filter(row=>row.type==='perf'&&Number.isFinite(Number(row.value)));for(const row of rows){byType[row.type]=(byType[row.type]||0)+1;if(row.type==='behavior'||row.type==='track')behavior[row.name]=(behavior[row.name]||0)+1}for(const metric of ['lcp','inp','fid','cls','fcp','fp','ttfb','longtask','white_screen','blank_screen_rate','first_screen','route_render','data_ready','dom_ready','page_load','js_boot','tbt','resource_failure_rate','slow_api_rate','dns','tcp','tls','request','download','cache_hit_rate','redirect','redirect_count']){const metricValues=perfRows.filter(row=>row.metric===metric).map(row=>Number(row.value)),value=metric.endsWith('_rate')&&metricValues.length?metricValues.reduce((sum,item)=>sum+item,0)/metricValues.length:percentile75(metricValues);if(value!==null)perf[metric]=Number(value.toFixed(metric==='cls'?4:0))}return json({totalEvents:rows.length,issueCount:issues.filter(row=>row.status!=='resolved').length,regressionCount:issues.filter(row=>row.status==='regression').length,lastSeen:rows[0]?.ts||null,perf,byType,behavior,api:aggregatePerf(perfRows.filter(row=>row.metric==='fetch'||row.metric==='xhr'),row=>row.props?.url||row.name||'unknown'),resources:aggregatePerf(perfRows.filter(row=>row.metric==='resource'),row=>row.props?.name||row.name||'unknown'),replays:[],alerts:[],issues:issues.map(mapIssue)})}
...
async function replayList(env,url){const page=Math.max(1,Number(url.searchParams.get('page')||1)),size=Math.min(100,Number(url.searchParams.get('pageSize')||10));const rows=await env.DB.prepare(`select session_id replayId,session_id,user_id userId,user_name userName,user_phone userPhone,max(created_at) lastSeen,max(url) url,max(release_name) release,max(end_reason) endReason,count(*) eventCount from replays group by session_id order by lastSeen desc limit ? offset ?`).bind(size,(page-1)*size).all();const total=await env.DB.prepare('select count(distinct session_id) count from replays').first();return json({items:rows.results,total:total.count,page,pageSize:size})}
async function replayEvents(env,id){const rows=(await env.DB.prepare('select events_json from replays where session_id=? order by created_at').bind(id).all()).results;return json(rows.flatMap(row=>parse(row.events_json,[])))}
async function traces(env,url){const {where,values}=filters(url,null,['trace_id is not null']);const rows=await env.DB.prepare(`select trace_id,min(ts) started_at,max(ts) ended_at,count(*) span_count,sum(case when type='error' or json_extract(props_json,'$.status')>=400 then 1 else 0 end) error_count,max(app_id) app_id,max(release_name) release_name,max(url) url from events ${where} group by trace_id order by started_at desc limit 200`).bind(...values).all();return json(rows.results.map(r=>({...r,duration:r.ended_at-r.started_at})))}
async function traceEvents(env,id){return json((await env.DB.prepare('select * from events where trace_id=? order by ts').bind(id).all()).results.map(mapEvent))}
async function sessions(env,url){const {where,values}=filters(url,null,['session_id is not null']);const rows=await env.DB.prepare(`select session_id,max(user_id) user_id,max(user_name) user_name,max(device_id) device_id,min(ts) started_at,max(ts) ended_at,count(*) event_count,sum(case when type='error' then 1 else 0 end) error_count,group_concat(distinct path) paths from events ${where} group by session_id order by ended_at desc limit 200`).bind(...values).all();const replayIds=(await env.DB.prepare('select distinct session_id from replays').all()).results;return json(rows.results.map(r=>({...r,duration:r.ended_at-r.started_at,paths:(r.paths||'').split(',').filter(Boolean),replaySessionId:replayIds.find(x=>x.session_id.startsWith(r.session_id))?.session_id})))}
async function sessionEvents(env,id){return json((await env.DB.prepare('select * from events where session_id=? order by ts limit 2000').bind(id).all()).results.map(mapEvent))}
...
async function exportCsv(env,kind){const table=kind==='replays'?'replays':kind,rows=(await env.DB.prepare(`select * from ${table} order by ${kind==='issues'?'last_seen':kind==='replays'?'created_at':'ts'} desc limit 10000`).all()).results;if(!rows.length)return new Response('',{headers:{'content-type':'text/csv'}});const keys=Object.keys(rows[0]),cell=v=>`"${String(v??'').replaceAll('"','""')}"`;return new Response('\ufeff'+[keys.map(cell).join(','),...rows.map(r=>keys.map(k=>cell(r[k])).join(','))].join('\r\n'),{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':`attachment; filename="web-collection-${kind}.csv"`}})}

function filters(url,forcedType,fixed=[],fixedValues=[]){const p=url.searchParams,parts=[...fixed],values=[...fixedValues];for(const [field,key,value] of [['app_id','appId'],['release_name','release'],['type','type',forcedType],['user_id','userId'],['session_id','sessionId']]){const v=value||p.get(key);if(v){parts.push(`${field}=?`);values.push(v)}}if(p.get('startTime')){parts.push('ts>=?');values.push(Number(p.get('startTime')))}if(p.get('endTime')){parts.push('ts<=?');values.push(Number(p.get('endTime')))}if(p.get('keyword')){parts.push('(name like ? or message like ? or props_json like ? or trace_id like ?)');values.push(...Array(4).fill(`%${p.get('keyword')}%`))}return{where:parts.length?`where ${parts.join(' and ')}`:'',values}}
function sanitize(event){const type=event.type==='performance'?'perf':String(event.type||'');if(!['track','perf','behavior','error','replay','log','trace'].includes(type))throw new Error('bad event type');const base={type,appId:clip(event.appId||'default',64),release:clip(event.release||'unknown',64),userId:clip(event.userId||'',128),userName:clip(event.userName||'',128),userPhone:clip(event.userPhone||'',32),sessionId:clip(event.sessionId||'',128),deviceId:clip(event.deviceId||'',128),traceId:clip(event.traceId||'',64),spanId:clip(event.spanId||'',32),url:cleanUrl(event.url),path:clip(event.path||'',512),title:clip(event.title||'',256),referrer:cleanUrl(event.referrer),userAgent:clip(event.userAgent||'',512),ts:Number(event.ts)||Date.now(),name:clip(event.name||'',160),metric:clip(event.metric||'',32),value:Number.isFinite(Number(event.value))?Number(event.value):null,message:redact(clip(event.message||'',500)),stack:clip(event.stack||'',4000),props:cleanObject(event.props),breadcrumbs:Array.isArray(event.breadcrumbs)?event.breadcrumbs.slice(-20).map(cleanObject):null};if(type==='replay')return{...base,events:Array.isArray(event.events)?event.events.slice(0,200):[],segmentEndReason:clip(event.segmentEndReason||'',32)};return base}
function mapEvent(r){return{id:r.id,ts:r.ts,type:r.type,appId:r.app_id,release:r.release_name,userId:r.user_id,userName:r.user_name,userPhone:maskPhone(r.user_phone),sessionId:r.session_id,deviceId:r.device_id,traceId:r.trace_id,spanId:r.span_id,url:r.url,path:r.path,title:r.title,referrer:r.referrer,userAgent:r.user_agent,name:r.name,metric:r.metric,value:r.value,message:r.message,stack:r.stack,props:parse(r.props_json,null),breadcrumbs:parse(r.breadcrumbs_json,null)}}
function mapIssue(r){return{fingerprint:r.fingerprint,status:r.status,appId:r.app_id,release:r.release_name,name:r.name,message:r.message,stack:r.stack,url:r.url,props:parse(r.props_json,null),breadcrumbs:parse(r.breadcrumbs_json,null),original:parse(r.original_json,null),count:r.count,firstSeen:r.first_seen,lastSeen:r.last_seen,resolvedAt:r.resolved_at,affectedUsers:0}}
