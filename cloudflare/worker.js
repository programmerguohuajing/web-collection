...
async function summary(env,url){const {where,values}=filters(url);const rows=(await env.DB.prepare(`select * from events ${where} order by ts desc limit 5000`).bind(...values).all()).results.map(mapEvent);const issues=(await env.DB.prepare('select * from issues order by last_seen desc limit 100').all()).results;const byType={},behavior={},performance={};for(const row of rows){byType[row.type]=(byType[row.type]||0)+1;if(row.type==='behavior'||row.type==='track')behavior[row.name]=(behavior[row.name]||0)+1;if(row.type==='perf')(performance[row.metric]||=[]).push(row.value)}return json({total:rows.length,errors:rows.filter(r=>r.type==='error').length,users:new Set(rows.map(r=>r.userId||r.deviceId).filter(Boolean)).size,replays:(await env.DB.prepare('select count(distinct session_id) count from replays').first()).count,byType,behavior,performance:Object.fromEntries(Object.entries(performance).map(([k,v])=>[k,{avg:v.reduce((a,b)=>a+b,0)/v.length,p95:v.sort((a,b)=>a-b)[Math.floor(v.length*.95)]} ])),issues:issues.map(mapIssue),events:rows})}
