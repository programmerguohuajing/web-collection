...
async function saveFunnel(env,input){const steps=strings(input.steps).slice(0,10);if(!input.name||steps.length<2)throw new Error('漏斗名称和至少两个步骤不能为空');const now=Date.now();const result=await env.DB.prepare('insert into funnel_definitions(name,app_id,steps_json,created_at,updated_at) values(?,?,?,?,?)').bind(clip(input.name,128),input.appId||null,JSON.stringify(steps),now,now).run();return json({id:result.meta.last_row_id})}
