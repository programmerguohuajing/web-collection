...
async function adminApi(request, env, url) {
  ...
  // 内部租户作用域只来自已校验会话，不接受客户端 query 伪造。
  url.__teamId = auth?.via === 'session' && auth.teamId ? auth.teamId : ''
  ...
  const appPathMatch = /^\/api\/applications\/([^/]+)/.exec(path)
  if (appPathMatch && auth?.via === 'session' && auth.teamId) {
    ...
    if (app.team_id !== auth.teamId) return json({ error: '无权访问该应用（跨团队）' }, 403)
  }
  ...
  if (/^\/api\/replays\//.test(path)) return replayEvents(env, decodeURIComponent(path.split('/').at(-1)), url)
  ...
  if (/^\/api\/traces\/[^/]+\/distributed$/.test(path)) return distributedTrace(env, decodeURIComponent(path.split('/').at(-2)), url)
  // BUG-013 修复：topology-plan F1 承诺的调用拓扑端点（服务端按「页面 → API」归并，与 Node getTraceTopology 同构）。
  if (/^\/api\/traces\/[^/]+\/topology$/.test(path)) return traceTopology(env, decodeURIComponent(path.split('/').at(-2)), url)
  ...
  if (/^\/api\/applications\/[^/]+$/.test(path) && request.method === 'DELETE') { const id=decodeURIComponent(path.split('/').at(-1)); for(const table of ['events','issues','replays','sourcemaps','alert_history','funnel_definitions','releases','experiment_exposures','experiments']) await env.DB.prepare(`delete from ${table} where app_id=?`).bind(id).run(); await env.DB.prepare('delete from applications where app_id=?').bind(id).run(); return json({ok:true}) }
  ...
  if (/\/issues\/[^/]+\/resolve$/.test(path) && request.method === 'POST') { const id=decodeURIComponent(path.split('/').at(-2)); const resNote=(await request.json().catch(()=>({})))?.resolutionNotes||null; const teamSql=url.__teamId?' and app_id in (select app_id from applications where team_id=?)':''; const teamVals=url.__teamId?[url.__teamId]:[]; await env.DB.prepare(`update issues set status='resolved',resolved_at=?,resolution_notes=coalesce(?,resolution_notes) where fingerprint=?${teamSql}`).bind(Date.now(),resNote,id,...teamVals).run(); return json(await env.DB.prepare(`select * from issues where fingerprint=?${teamSql}`).bind(id,...teamVals).first()) }
  if (path === '/api/sourcemaps' && request.method === 'POST') { const body=await request.json(); if(url.__teamId){const app=await q1(env,'select team_id from applications where app_id=?',[String(body?.appId||'default').slice(0,64)]); if(!app||app.team_id!==url.__teamId)return json({error:'无权写入该应用 SourceMap（跨团队）'},403)} return saveSourceMap(env, body) }
  if (path === '/api/funnels' && request.method === 'GET') return funnelList(env, url)
  if (path === '/api/funnels' && request.method === 'POST') { const body=await request.json(); if(url.__teamId){const app=await q1(env,'select team_id from applications where app_id=?',[String(body?.appId||'').slice(0,64)]); if(!app||app.team_id!==url.__teamId)return json({error:'无权创建该应用漏斗（跨团队）'},403)} return saveFunnel(env, body) }
  if (/^\/api\/funnels\/\d+$/.test(path) && request.method === 'DELETE') { const id=Number(path.split('/').at(-1)); if(url.__teamId) await env.DB.prepare('delete from funnel_definitions where id=? and app_id in (select app_id from applications where team_id=?)').bind(id,url.__teamId).run(); else await env.DB.prepare('delete from funnel_definitions where id=?').bind(id).run(); return json({ok:true}) }
  if (path === '/api/dashboards' && request.method === 'GET') { const rows=url.__teamId?await env.DB.prepare('select * from dashboards where team_id=? order by updated_at desc').bind(url.__teamId).all():await env.DB.prepare('select * from dashboards order by updated_at desc').all(); return json(rows.results.map(row => ({...row,widgets_json:parse(row.widgets_json,[])}))) }
  if (path === '/api/dashboards' && request.method === 'POST') return saveDashboard(env, await request.json(), url.__teamId || '')
  if (/^\/api\/dashboards\/\d+$/.test(path) && request.method === 'DELETE') { const id=Number(path.split('/').at(-1)); if(url.__teamId) await env.DB.prepare('delete from dashboards where id=? and team_id=?').bind(id,url.__teamId).run(); else await env.DB.prepare('delete from dashboards where id=?').bind(id).run(); return json({ok:true}) }
  // A2 · 自定义看板分享：分享/取消走现有鉴权通道（与 adminApi 同口径，无独立门禁）。
  if (/^\/api\/dashboards\/\d+\/share$/.test(path) && request.method === 'POST') return shareDashboard(env, Number(path.split('/')[3]), url.__teamId || '')
  if (/^\/api\/dashboards\/\d+\/share$/.test(path) && request.method === 'DELETE') return unshareDashboard(env, Number(path.split('/')[3]), url.__teamId || '')
...
async function traceEvents(env,id,url){const page=Math.max(1,Number(url.searchParams.get('page')||1)),pageSize=Math.min(100,Math.max(1,Number(url.searchParams.get('pageSize')||10)));if(!id?.trim())return json({items:[],total:0,page,pageSize});const{where,values}=filters(url,null,['trace_id=?'],[id]);const[rows,total]=await Promise.all([env.DB.prepare(`select * from events ${where} order by ts limit ? offset ?`).bind(...values,pageSize,(page-1)*pageSize).all(),env.DB.prepare(`select count(*) count from events ${where}`).bind(...values).first()]);return json({items:rows.results.map(mapEvent),total:Number(total.count),page,pageSize})}
async function distributedTrace(env,id,url){if(!id?.trim())return json({root:null,nodes:[],edges:[],criticalPath:[],errorSpans:[]});const{where,values}=filters(url,null,['trace_id=?'],[id]);const events=await env.DB.prepare(`select * from events ${where} order by ts asc limit ?`).bind(...values,MAX_TRACE_EVENTS).all();const eventRows=events.results||[];const backendSpans=eventRows.length&&!url.__teamId?await env.DB.prepare('select * from spans where trace_id=? order by start_ts asc limit ?').bind(id,MAX_TRACE_EVENTS).all().catch(()=>({results:[]})):{results:[]};return json(buildDistributedTrace(eventRows,backendSpans.results||[]))}
...
async function traceTopology(env,id,url){
  if(!id?.trim())return json({nodes:[],edges:[]})
  const {where,values}=filters(url,null,['trace_id=?'],[id])
  const events=await env.DB.prepare(`select * from events ${where} order by ts limit 5000`).bind(...values).all().catch(()=>({results:[]}))
...
async function releasesReport(env,url){const appId=url.searchParams.get('appId'),release=url.searchParams.get('release'),startTime=url.searchParams.get('startTime'),endTime=url.searchParams.get('endTime');let sql='select r.app_id,r.release_name release,r.status,r.created_at,count(e.id) events,sum(case when e.type=\'error\' then 1 else 0 end) errors,count(distinct coalesce(nullif(e.user_id,\'\'),e.device_id)) users,round(avg(case when e.type=\'perf\' and e.metric=\'lcp\' then e.value end),2) lcp from releases r left join events e on e.app_id=r.app_id and e.release_name=r.release_name';const params=[];if(startTime){sql+=' and e.ts>=?';params.push(Number(startTime))}if(endTime){sql+=' and e.ts<=?';params.push(Number(endTime))}const where=[];if(url.__teamId){where.push('r.app_id in (select app_id from applications where team_id=?)');params.push(url.__teamId)}if(appId){where.push('r.app_id=?');params.push(appId)}if(release){where.push('r.release_name=?');params.push(release)}sql+=where.length?` where ${where.join(' and ')}`:' where 1=1';sql+=' group by r.app_id,r.release_name,r.status,r.created_at order by r.created_at desc limit 20';return json((await env.DB.prepare(sql).bind(...params).all()).results)}
async function funnelEventNames(env,url){const scoped=new URL(url);scoped.__teamId=url.__teamId||'';scoped.searchParams.delete('type');scoped.searchParams.delete('name');const{where,values}=filters(scoped,null,["type in ('behavior','track')","name<>''"]);return json((await env.DB.prepare(`select name,count(*) count from events ${where} group by name order by count desc,name limit 100`).bind(...values).all()).results)}
async function funnelList(env,url){const page=Math.max(1,Number(url.searchParams.get('page')||1)),pageSize=Math.min(100,Math.max(1,Number(url.searchParams.get('pageSize')||10))),where=url.__teamId?' where app_id in (select app_id from applications where team_id=?)':'',values=url.__teamId?[url.__teamId]:[],[rows,total]=await Promise.all([env.DB.prepare(`select * from funnel_definitions${where} order by updated_at desc limit ? offset ?`).bind(...values,pageSize,(page-1)*pageSize).all(),env.DB.prepare(`select count(*) count from funnel_definitions${where}`).bind(...values).first()]);return json({items:rows.results.map(row=>({...row,steps_json:parse(row.steps_json,[])})),total:Number(total.count),page,pageSize})}
...
async function runFunnel(env,id,url){const def=url.__teamId?await env.DB.prepare('select * from funnel_definitions where id=? and app_id in (select app_id from applications where team_id=?)').bind(id,url.__teamId).first():await env.DB.prepare('select * from funnel_definitions where id=?').bind(id).first();if(!def)throw new Error('漏斗不存在');const scoped=new URL(url);scoped.__teamId=url.__teamId||'';if(def.app_id&&!scoped.searchParams.get('appId'))scoped.searchParams.set('appId',def.app_id);const steps=parse(def.steps_json,[]),windowMs=def.window_ms?Number(def.window_ms):null,{where,values}=filters(scoped);const rows=(await env.DB.prepare(`select session_id,coalesce(nullif(user_id,''),device_id,session_id) actor,name,type,ts,release_name,case when user_agent like '%Edg/%' then 'Edge' when user_agent like '%Chrome/%' then 'Chrome' when user_agent like '%Firefox/%' then 'Firefox' when user_agent like '%Safari/%' then 'Safari' else 'Other' end browser,case when user_agent like '%Mobile%' then 'Mobile' else 'Desktop' end device from events ${where} ${where?'and':'where'} (name in (${steps.map(()=>'?').join(',')}) or type='error') order by ts limit 50000`).bind(...values,...steps).all()).results,actors=Object.values(group(rows,r=>r.actor)),matched=actors.map(a=>matchSteps(a,steps,windowMs)),counts=steps.map((_,i)=>matched.filter(m=>m[i]!=null).length),ttcByStep=steps.map(()=>[]);matched.forEach(m=>{for(let i=1;i<m.length;i++)if(m[i]!=null&&m[i-1]!=null)ttcByStep[i].push(m[i]-m[i-1])});const median=v=>{if(!v.length)return null;const s=[...v].sort((a,b)=>a-b),m=s.length>>1;return s.length%2?s[m]:Math.round((s[m-1]+s[m])/2)};const p90=v=>{if(!v.length)return null;const s=[...v].sort((a,b)=>a-b);return s[Math.min(s.length-1,Math.max(0,Math.ceil(0.9*s.length)-1))]};const replayTeam=url.__teamId?' where app_id in (select app_id from applications where team_id=?)':'',replayVals=url.__teamId?[url.__teamId]:[],replayIds=(await env.DB.prepare(`select distinct session_id, base_session_id from replays${replayTeam}`).bind(...replayVals).all()).results;const lostSessions=actors.filter(a=>reaches(a,steps,0,windowMs)&&!reaches(a,steps,steps.length-1,windowMs)).slice(0,100).map(a=>({sessionId:a[0].session_id,actor:a[0].actor,lastEvent:a.filter(x=>x.type!=='error').at(-1)?.name,errors:a.filter(x=>x.type==='error').length,replaySessionId:replayIds.find(x=>x.base_session_id===a[0].session_id)?.session_id}));return json({definition:def,steps:steps.map((step,i)=>({step,count:counts[i],rate:counts[0]?Number((counts[i]/counts[0]*100).toFixed(2)):0,stepRate:i?counts[i-1]?Number((counts[i]/counts[i-1]*100).toFixed(2)):0:100,lost:i?counts[i-1]-counts[i]:0,timeToConvert:i?median(ttcByStep[i]):null,timeToConvertP90:i?p90(ttcByStep[i]):null})),lostSessions,dimensions:['release_name','browser','device'].map(field=>({field,items:dimensions(rows,steps,field,windowMs)})),trend:dimensions(rows,steps,'day',windowMs),windowMs})}
async function saveDashboard(env,input,teamId=''){if(!input.name)throw new Error('仪表盘名称不能为空');const now=Date.now(),widgets=Array.isArray(input.widgets)?input.widgets.slice(0,12).map(item=>{if(typeof item==='string')return item.trim().slice(0,64);const type=item&&item.type==='funnel'?'funnel':item&&item.type==='insight'?'insight':'';const id=Number(item&&item.id);return type&&Number.isInteger(id)&&id>0?{type,id}:null}).filter(Boolean):[];const result=await env.DB.prepare('insert into dashboards(name,widgets_json,team_id,created_at,updated_at) values(?,?,?,?,?)').bind(clip(input.name,128),JSON.stringify(widgets),teamId||null,now,now).run();return json({id:result.meta.last_row_id})}
// A2 · 自定义看板分享：生成唯一分享 token 并标记 shared=1；幂等（重生成令旧链接失效）。
async function shareDashboard(env, id, teamId='') {
  const token = crypto.randomUUID()
  if(teamId) await env.DB.prepare('update dashboards set shared=1, share_token=? where id=? and team_id=?').bind(token, Number(id), teamId).run()
  else await env.DB.prepare('update dashboards set shared=1, share_token=? where id=?').bind(token, Number(id)).run()
  const row = teamId ? await env.DB.prepare('select id, name, widgets_json from dashboards where id=? and team_id=?').bind(Number(id), teamId).first() : await env.DB.prepare('select id, name, widgets_json from dashboards where id=?').bind(Number(id)).first()
  if (!row) return new Response('not found', { status: 404 })
  return json({ id: Number(row.id), shared: true, shareToken: token })
}
// A2 · 自定义看板分享：取消分享，清除 token 使旧链接立即失效。
async function unshareDashboard(env, id, teamId='') {
  if(teamId) await env.DB.prepare('update dashboards set shared=0, share_token=null where id=? and team_id=?').bind(Number(id), teamId).run()
  else await env.DB.prepare('update dashboards set shared=0, share_token=null where id=?').bind(Number(id)).run()
...
function accountsEnforced(env) { return env.ACCOUNTS_ENFORCE !== '0' && env.ACCOUNTS_ENFORCE !== 'false' }
...
function jwtSecretOf(env) {
  ...
  throw new Error('ACCOUNTS_JWT_SECRET 未配置，账号体系无法签发或校验访问令牌')
}
...
async function resolveAuth(request, env) {
  ...
  if (bearer) {
    ...
    if (payload?.sub && payload.sid) {
      ...
      if (session && session.user_id === payload.sub && !session.revoked_at && Number(session.expires_at) >= Date.now()) {
...
async function teamAssignApplication(request, env, auth, teamId) {
  ...
  const app = await q1(env, 'select app_id,team_id from applications where app_id=?', [appId])
  if (!app) return json({ error: '应用不存在' }, 404)
  if (app.team_id && app.team_id !== teamId) return json({ error: '应用已归属其他团队，禁止跨团队接管' }, 403)
...
export function filters(url,forcedType,fixed=[],fixedValues=[]){
  const p=url.searchParams,parts=[...fixed],values=[...fixedValues]
  if(url.__teamId){parts.push('app_id in (select app_id from applications where team_id=?)');values.push(url.__teamId)}
  const _hasTs=fixed.some(f=>/\bts\s*[<>]=?\s*\?/.test(f))||p.has('startTime')||p.has('endTime')
  if(!_hasTs){parts.push('ts>=?');values.push(Date.now()-90*86400000)}
  for(const [field,key,value] of [['app_id','appId'],['release_name','release'],['type','type',forcedType],['name','name'],['user_id','userId'],['session_id','sessionId']]){const v=value||p.get(key);if(v){const items=field==='type'?String(v).split(',').filter(Boolean):[v];parts.push(items.length>1?`${field} in (${items.map(()=>'?').join(',')})`:`${field}=?`);values.push(...items)}}
  if(p.get('traceId')){parts.push('trace_id like ?');values.push(`%${p.get('traceId')}%`)}
  if(p.get('path')){parts.push('(path like ? or url like ?)');values.push(...Array(2).fill(`%${p.get('path')}%`))}
  if(p.get('startTime')){parts.push('ts>=?');values.push(Number(p.get('startTime')))}
  if(p.get('endTime')){parts.push('ts<=?');values.push(Number(p.get('endTime')))}
  if(p.get('keyword')){parts.push('(name like ? or message like ? or props_json like ? or trace_id like ?)');values.push(...Array(4).fill(`%${p.get('keyword')}%`))}
  return{where:parts.length?`where ${parts.join(' and ')}`:'',values}
}
export function issueFilters(url){
  const p=url.searchParams,parts=[],values=[]
  if(url.__teamId){parts.push('app_id in (select app_id from applications where team_id=?)');values.push(url.__teamId)}
  if(!p.has('startTime')&&!p.has('endTime')){parts.push('last_seen>=?');values.push(Date.now()-90*86400000)}
  for(const[field,key]of[['app_id','appId'],['release_name','release'],['status','status']]){if(p.get(key)){parts.push(`${field}=?`);values.push(p.get(key))}}
  if(p.get('path')){parts.push('url like ?');values.push(`%${p.get('path')}%`)}
  if(p.get('startTime')){parts.push('last_seen>=?');values.push(Number(p.get('startTime')))}
  if(p.get('endTime')){parts.push('last_seen<=?');values.push(Number(p.get('endTime')))}
  if(p.get('keyword')){parts.push('(name like ? or message like ? or stack like ? or props_json like ?)');values.push(...Array(4).fill(`%${p.get('keyword')}%`))}
  return{where:parts.length?`where ${parts.join(' and ')}`:'',values}
}
export function replayFilters(url){
  const p=url.searchParams,parts=[],values=[]
  if(url.__teamId){parts.push('app_id in (select app_id from applications where team_id=?)');values.push(url.__teamId)}
  if(!p.has('startTime')&&!p.has('endTime')){parts.push('created_at>=?');values.push(Date.now()-90*86400000)}
  for(const[field,key]of[['app_id','appId'],['release_name','release'],['user_id','userId']]){if(p.get(key)){parts.push(`${field}=?`);values.push(p.get(key))}}
  for(const[field,key]of[['user_name','userName'],['user_phone','userPhone'],['url','path']]){if(p.get(key)){parts.push(`${field} like ?`);values.push(`%${p.get(key)}%`)}}
  if(p.get('startTime')){parts.push('created_at>=?');values.push(Number(p.get('startTime')))}
  if(p.get('endTime')){parts.push('created_at<=?');values.push(Number(p.get('endTime')))}
  if(p.get('keyword')){parts.push('(session_id like ? or url like ? or events_json like ?)');values.push(...Array(3).fill(`%${p.get('keyword')}%`))}
  return{where:parts.length?`where ${parts.join(' and ')}`:'',values}
}
