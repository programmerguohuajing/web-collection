...
async function releasesReport(env,url){const appId=url.searchParams.get('appId'),startTime=url.searchParams.get('startTime'),endTime=url.searchParams.get('endTime');let sql='select r.app_id,r.release_name release,r.created_at,count(e.id) events,sum(case when e.type=\'error\' then 1 else 0 end) errors,count(distinct coalesce(nullif(e.user_id,\'\'),e.device_id)) users,round(avg(case when e.type=\'perf\' and e.metric=\'lcp\' then e.value end),2) lcp from releases r left join events e on e.app_id=r.app_id and e.release_name=r.release_name';const params=[];if(startTime){sql+=' and e.ts>=?';params.push(Number(startTime))}if(endTime){sql+=' and e.ts<=?';params.push(Number(endTime))}if(appId){sql+=' where r.app_id=?';params.push(appId)}else{sql+=' where 1=1'}sql+=' group by r.app_id,r.release_name order by r.created_at desc limit 20';return json(await env.DB.prepare(sql).bind(...params).all())}
