...
async function record(env, event, application) {
  ...
  if (event.type === 'replay') {
    if (event.sessionId && event.events?.length) await storageWrite(env, `insert into replays (session_id,app_id,user_id,user_name,user_phone,created_at,url,release_name,end_reason,events_json,base_session_id) values (?,?,?,?,?,?,?,?,?,?,?)`, [event.sessionId,event.appId,event.userId,event.userName,event.userPhone,event.ts,event.url,event.release,event.segmentEndReason||null,JSON.stringify(event.events),event.baseSessionId||null])
    return true
  }
